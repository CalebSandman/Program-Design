...
int main()
{
    ...
    printf("Please select from the four types of storages\n");
    printf("1: 5x5\n");
    printf("2: 5x10\n");
    printf("3: 10x10\n");
    printf("4: 10x15\n");
    ...
    //user input for size
    printf("Please select a storage type: ");
    scanf("%d", &size_input);
    if(size_input < 1 || size_input > 4)    //if not in range exit application
    {
        printf("Invalid size. Select from 1-4");
        return 0;
    }

    //user input for months
    printf("Please enter the months you need: ");
    scanf("%d", &months_input);
    if(months_input < 1)    //if not <= 1 exit application
    {
        printf("Invalid time. Please anter a number greater than 0");
        return 0;
    }
